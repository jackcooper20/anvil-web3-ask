...
export class Anvil {
  ...
  public async stopImpersonatingAccount(account: ValidAddress): Promise<void> {
    await this.sendAnvilMethod(AnvilRPC.anvil_stopImpersonatingAccount, [
      account,
    ]);
  }

  public async autoImpersonateAccount(enabled: boolean): Promise<void> {
    await this.sendAnvilMethod(AnvilRPC.anvil_autoImpersonateAccount, [
      enabled,
    ]);
  }
  ...
  public async setStorageAt(
    ...
    return await this.sendAnvilMethod(AnvilRPC.anvil_setStorageAt, [
      address,
      slot,
      val,
    ]);
  ...
  public async setNextBlockTimestamp(timestamp: number): Promise<void> {
    await this.sendAnvilMethod(AnvilRPC.evm_setNextBlockTimestamp, [timestamp]);
  }
